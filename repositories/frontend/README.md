<!-- GENERATED FILE — DO NOT EDIT BY HAND.
     Regenerate: python3 scripts/generate-index/generate_repository_cards.py -->

# Frontend

54 repositories · 53 usable · 1 archived or abandoned ·
all facts verified against the GitHub API on 2026-09-15.

## Highest trust

| Repository | Stars | Tier | Status | License | Trust | Verified |
|---|---|---|---|---|---|---|
| [lobehub/lobehub](./lobehub--lobehub.md) | 82,498 | A | 🟢 ACTIVE | `NOASSERTION` | 9.22 | 2026-09-15 |
| [calcom/cal.diy](./calcom--cal.diy.md) | 48,481 | S | 🟢 ACTIVE | `MIT` | 9.2 | 2026-09-15 |
| [payloadcms/payload](./payloadcms--payload.md) | 44,751 | S | 🟢 ACTIVE | `MIT` | 8.98 | 2026-09-15 |
| [mui/base-ui](./mui--base-ui.md) | 10,906 | S | 🟢 ACTIVE | `MIT` | 8.97 | 2026-09-15 |
| [nuxt/nuxt](./nuxt--nuxt.md) | 60,864 | S | 🟢 ACTIVE | `MIT` | 8.95 | 2026-09-15 |
| [pmndrs/zustand](./pmndrs--zustand.md) | 58,689 | S | 🟢 ACTIVE | `MIT` | 8.85 | 2026-09-15 |
| [pmndrs/drei](./pmndrs--drei.md) | 9,869 | S | 🟢 ACTIVE | `MIT` | 8.85 | 2026-09-15 |
| [xyflow/xyflow](./xyflow--xyflow.md) | 38,382 | S | 🟢 ACTIVE | `MIT` | 8.79 | 2026-09-15 |
| [TanStack/table](./tanstack--table.md) | 28,429 | S | 🟢 ACTIVE | `MIT` | 8.71 | 2026-09-15 |
| [appsmithorg/appsmith](./appsmithorg--appsmith.md) | 40,878 | S | 🟢 ACTIVE | `Apache-2.0` | 8.7 | 2026-09-15 |
| [vercel/next.js](./vercel--next.js.md) | 142,318 | S | 🟢 ACTIVE | `MIT` | 8.62 | 2026-09-15 |
| [storybookjs/storybook](./storybookjs--storybook.md) | 91,053 | S | 🟢 ACTIVE | `MIT` | 8.6 | 2026-09-15 |
| [open-webui/open-webui](./open-webui--open-webui.md) | 152,147 | A | 🟢 ACTIVE | `NOASSERTION` | 8.55 | 2026-09-15 |
| [mrdoob/three.js](./mrdoob--three.js.md) | 115,552 | S | 🟢 ACTIVE | `MIT` | 8.54 | 2026-09-15 |
| [recharts/recharts](./recharts--recharts.md) | 27,557 | S | 🟢 ACTIVE | `MIT` | 8.5 | 2026-09-15 |

## Do not adopt

| Repository | Stars | Status | Days since push |
|---|---|---|---|
| [rsms/inter](./rsms--inter.md) | 19,885 | 🔴 ABANDONED | 665d |

## Cards

- [apache/echarts](./apache--echarts.md) — tier S, ACTIVE
- [appsmithorg/appsmith](./appsmithorg--appsmith.md) — tier S, ACTIVE
- [calcom/cal.diy](./calcom--cal.diy.md) — tier S, ACTIVE
- [colinhacks/zod](./colinhacks--zod.md) — tier S, ACTIVE
- [d3/d3](./d3--d3.md) — tier A, STABLE
- [documenso/documenso](./documenso--documenso.md) — tier S, ACTIVE
- [dubinc/dub](./dubinc--dub.md) — tier A, ACTIVE
- [fontsource/fontsource](./fontsource--fontsource.md) — tier S, ACTIVE
- [formbricks/formbricks](./formbricks--formbricks.md) — tier A, ACTIVE
- [google-gemini/gemini-fullstack-langgraph-quickstart](./google-gemini--gemini-fullstack-langgraph-quickstart.md) — tier B, STABLE
- [greensock/GSAP](./greensock--gsap.md) — tier UNVERIFIED, MAINTENANCE
- [lobehub/lobehub](./lobehub--lobehub.md) — tier A, ACTIVE
- [magicuidesign/magicui](./magicuidesign--magicui.md) — tier A, ACTIVE
- [makeplane/plane](./makeplane--plane.md) — tier A, ACTIVE
- [microsoft/TypeScript](./microsoft--typescript.md) — tier S, ACTIVE
- [midday-ai/midday](./midday-ai--midday.md) — tier A, STABLE
- [motiondivision/motion](./motiondivision--motion.md) — tier S, ACTIVE
- [mrdoob/three.js](./mrdoob--three.js.md) — tier S, ACTIVE
- [mui/base-ui](./mui--base-ui.md) — tier S, ACTIVE
- [nocodb/nocodb](./nocodb--nocodb.md) — tier A, ACTIVE
- [nuxt/nuxt](./nuxt--nuxt.md) — tier S, ACTIVE
- [observablehq/plot](./observablehq--plot.md) — tier S, ACTIVE
- [open-webui/open-webui](./open-webui--open-webui.md) — tier A, ACTIVE
- [payloadcms/payload](./payloadcms--payload.md) — tier S, ACTIVE
- [pmndrs/drei](./pmndrs--drei.md) — tier S, ACTIVE
- [pmndrs/react-three-fiber](./pmndrs--react-three-fiber.md) — tier S, ACTIVE
- [pmndrs/zustand](./pmndrs--zustand.md) — tier S, ACTIVE
- [radix-ui/primitives](./radix-ui--primitives.md) — tier A, STABLE
- [react/react](./react--react.md) — tier A, ACTIVE
- [recharts/recharts](./recharts--recharts.md) — tier S, ACTIVE
- [refinedev/refine](./refinedev--refine.md) — tier S, ACTIVE
- [rolldown/rolldown](./rolldown--rolldown.md) — tier S, ACTIVE
- [rsms/inter](./rsms--inter.md) — tier C, ABANDONED
- [shadcn-ui/taxonomy](./shadcn-ui--taxonomy.md) — tier B, MAINTENANCE
- [shadcn-ui/ui](./shadcn-ui--ui.md) — tier S, ACTIVE
- [solidjs/solid](./solidjs--solid.md) — tier S, ACTIVE
- [storybookjs/storybook](./storybookjs--storybook.md) — tier S, ACTIVE
- [sveltejs/svelte](./sveltejs--svelte.md) — tier S, ACTIVE
- [t3-oss/create-t3-app](./t3-oss--create-t3-app.md) — tier B, MAINTENANCE
- [tailwindlabs/headlessui](./tailwindlabs--headlessui.md) — tier A, MAINTENANCE
- [tailwindlabs/tailwindcss](./tailwindlabs--tailwindcss.md) — tier A, ACTIVE
- [TanStack/query](./tanstack--query.md) — tier S, ACTIVE
- [TanStack/router](./tanstack--router.md) — tier S, ACTIVE
- [TanStack/table](./tanstack--table.md) — tier S, ACTIVE
- [tremorlabs/tremor](./tremorlabs--tremor.md) — tier B, MAINTENANCE
- [vercel/chatbot](./vercel--chatbot.md) — tier A, STABLE
- [vercel/commerce](./vercel--commerce.md) — tier A, STABLE
- [vercel/geist-font](./vercel--geist-font.md) — tier A, STABLE
- [vercel/next.js](./vercel--next.js.md) — tier S, ACTIVE
- [vercel/turborepo](./vercel--turborepo.md) — tier S, ACTIVE
- [vitejs/vite](./vitejs--vite.md) — tier S, ACTIVE
- [vuejs/core](./vuejs--core.md) — tier S, ACTIVE
- [withastro/astro](./withastro--astro.md) — tier A, ACTIVE
- [xyflow/xyflow](./xyflow--xyflow.md) — tier S, ACTIVE
